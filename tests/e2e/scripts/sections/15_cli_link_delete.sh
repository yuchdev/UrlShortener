...
#
# E2E CLI section 15: `link delete` must exit promptly without ever binding a
# listening socket (plan.md C4; Task 03.0 subtask 03).
#
# Mirrors 13_cli_no_server_socket.sh's port-check pattern for the delete verb.
...
# (exit code 1 plus a "delete failed: Link not found" diagnostic on stderr).
...
# "delete failed: Link not found" diagnostic). Any other outcome (e.g. an
# argument-parsing error before dispatch) is a real failure of this section.
if [[ "$rc" -ne 1 ]] || ! grep -q "delete failed: Link not found" "$stderr_file"; then
  echo "FAIL: 'link delete' did not reach the dispatch not-found result (rc=$rc)." >&2
  cat "$stderr_file" >&2
  exit 1
fi
...
if _port_open 8000; then
  echo "FAIL: port 8000 is OPEN after 'link delete' exited." >&2
  echo "  The binary started the HTTP server in CLI mode." >&2
  exit 1
fi

echo "PASS: 15_cli_link_delete"
