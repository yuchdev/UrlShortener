...
## Required Components
- `qa/scripts/sections/05_fingerprinting.sh`
...
```bash
bash qa/scripts/sections/05_fingerprinting.sh
```
...
```bash
python3 -m tools.sqlite_state_assert --db qa/tmp/qa.sqlite --expect qa/expected/05_fingerprinting.json
```

### Phase 6 — Verify Runtime State Again
```bash
python3 -m tools.sqlite_state_assert --assert-log-contains qa/tmp/server.log suspicious
```
