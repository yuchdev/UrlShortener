...
#
# E2E CLI section 17: `link restore` must exit promptly without ever binding a
...
if ! timeout 5 "$BINARY" link restore \
...
if _port_open 8000; then
  echo "FAIL: port 8000 is OPEN after 'link restore' exited." >&2
  echo "  The binary started the HTTP server in CLI mode." >&2
  exit 1
fi

echo "PASS: 17_cli_link_restore"
