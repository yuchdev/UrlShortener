...
```bash
bash tests/e2e/scripts/sections/09_error_handling.sh
```
...
```bash
python3 -m tools.sqlite_state_assert --db tests/e2e/tmp/qa.sqlite --expect tests/e2e/expected/09_error_handling.json
```

### Phase 6 — Verify Runtime State Again
```bash
python3 -m tools.sqlite_state_assert --assert-log-contains tests/e2e/tmp/server.log duplicate
```
