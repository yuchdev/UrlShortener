...
```bash
bash tests/e2e/scripts/sections/02_create_short_url.sh
```
...
```bash
python3 -m tools.sqlite_state_assert --db tests/e2e/tmp/qa.sqlite --expect tests/e2e/expected/02_create_short_url.json
```
