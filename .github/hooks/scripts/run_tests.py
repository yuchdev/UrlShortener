#!/usr/bin/env python3
"""agentStop hook - gate the end of a session on format + build + tests
(Copilot CLI port).
...
If any step fails, the hook returns a ``block`` decision so the agent is told to
analyze that tool's own output and fix what it flagged before ending the session
- not to weaken the check. To avoid infinite loops the hook suppresses itself
when the stop was itself triggered by a prior stop-hook block
(``stop_hook_active``).
...
skips gracefully (allow) rather than blocking - configure a build dir to arm it.
"""
...
from _common import REPO_ROOT, allow, append_log, block_stop, read_event
