...
- The **`.claude/hooks/post_edit_format.py`** PostToolUse hook auto-runs
  clang-format on files you edit — it is a safety net, not a substitute for
  running clang-tidy and fixing diagnostics yourself.
- The **`.claude/hooks/secret_scan.py`** PreToolUse hook blocks any write that
...
- The **`.claude/hooks/run_tests.py`** Stop hook re-runs clang-format + a build +
  ...
  broken.
- The **`.claude/hooks/guard_bash.py`** PreToolUse hook blocks destructive/prod
