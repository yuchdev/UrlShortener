...
## Required Components
- `qa/scripts/sections/10_concurrency.sh`
...
```bash
bash qa/scripts/sections/10_concurrency.sh
```
...
```bash
python3 -m tools.sqlite_state_assert --db qa/tmp/qa.sqlite --expect qa/expected/10_concurrency.json
```

### Phase 6 — Verify Runtime State Again
```bash
python3 -m tools.sqlite_state_assert --assert-log-contains qa/tmp/server.log concurrency
```
