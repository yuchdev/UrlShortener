...
> Milestone numbers `0003` and `0004` are unused/reserved - there is no
> record of what, if anything, was planned for them. Numbering was left as-is
> during the 2026-09-21 roadmap restructuring rather than renumbering `0005`,
> since that would have rippled into its existing cross-references.
>
> All three milestones are decomposed into the full task → subtask tree.
> Milestone 0001 is **fully implemented** (see its
> [status.md](/docs/roadmap/0001-rest-api-refactoring/status.md) for verified
> test/build evidence). Milestones 0002 and 0005 are fully specified but
> **not started** - see each one's `status.md` for open risks, and note that
> 0005's plan.md carries an unresolved verification question (no existing
> Linux `secret-store` implementation could be located in this repository to
> confirm parity against).
