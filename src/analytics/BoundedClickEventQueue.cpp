...
EnqueueResult BoundedClickEventQueue::TryEnqueue(ClickEvent event) noexcept {
    ...
    try {
        ...
        ++dropped_total_; return EnqueueResult::DroppedFull;
    }
    ++enqueued_total_; cv_.notify_one(); return EnqueueResult::Enqueued;
}
...
bool BoundedClickEventQueue::WaitDequeue(ClickEvent& out, std::chrono::milliseconds timeout) noexcept {
    ...
    if (!cv_.wait_for(lock, timeout, [this]{ return !queue_.empty() || stop_; })) return false;
    if (queue_.empty()) return false;
    out = std::move(queue_.front()); queue_.pop_front(); return true;
}
...
void BoundedClickEventQueue::NotifyStop() noexcept {
